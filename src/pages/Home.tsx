...
const Home = () => {
  ...
  const [recentHistory, setRecentHistory] = useState<ScanEntry[]>([]);

  // Simulate async data fetch (like calling an API)
  useEffect(() => {
    const fetchData = () => {
      const mapped: ScanEntry[] = MOCK_SCAN_HISTORY.map((item, index) => ({
        id: index + 1,
    ...
    const timeout = setTimeout(fetchData, 800); // Simulated latency
