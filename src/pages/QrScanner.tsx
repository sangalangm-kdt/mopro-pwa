...
const QRScanner = () => {
    ...
    const product = projects?.flatMap((item) => item.products);
    console.log(product);
    ...
    const {
        ...
        onResult: (data) => {
            const found = product.find((item) => item.lineNumber == data);
            console.log(data, product[2].id);

            if (!found) {
                toast.error(
                    "No matching record found for the scanned QR code."
                );
                handleRescan();
                return;
            }
                ...
                <ManualEntryModal
                    ...
                    onSubmit={({ drawingNumber }) => {
                        const found = product.find(
                            (item) => item.lineNumber == drawingNumber
                        );
